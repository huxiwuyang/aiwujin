		...
		onLaunch: function() {
			...
			if (width > 800) {
				if (window.self === window.top) {
					window.location.replace('https://www.shengqianxiong.com.cn');
				}
			}
			...
			if (ua.indexOf('micromessenger') !== -1) {
				...
				if (!openid) {
					if (window.location.href.indexOf('?code=') !== -1 || window.location.href.indexOf('&code=') !== -1) {
						...
						this.$Request.get('/tao/wx/openId/' + code).then(res => {
							if (res && res != '') {
								...
								this.$Request.getT('/user/openid/login/' + res).then(res => {
									if (res.status === 0) {
						...
						if (
							window.location.href.indexOf('/pages/search/search') === -1 ||
							window.location.href.indexOf('/pages/detail/goodsinfo') === -1 ||
							window.location.href.indexOf('/pages/member/publisher') === -1
						) {
					...
					this.$Request.getT('/user/openid/login/' + openid).then(res => {
						if (res.status === 0) {
							this.$queue.setData('token', res.data.uuid);
							this.$queue.setData('userId', res.data.userId);
							this.$queue.setData('mobile', res.data.phone);
