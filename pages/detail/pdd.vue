		...
		<view class="index-goods" v-if="loadGoods">
			...
			<view class="goods_info" style="padding-top: 6px;padding-bottom: 10px">
				<view style="padding: 8px">
					<image style="height: 26upx;width:26upx;margin-top: 4px;margin-right: 4px" src="../../static/img/pinduoduo.png"></image>
				...
				<view class="coupon-price s-row">
					<view class="price" style="width: 70%">
						...
						<text style="font-size: 12px;color: grey;text-decoration:line-through;margin-left: 8px">￥{{
                            ((goods.minGroupPrice)*0.01).toFixed(2)}}
						</text>
					</view>
					<view class="volume" style="padding-top: 14rpx;font-size: 14px;width: 30%">{{ goods.salesTip }}人付款
					</view>
				</view>
			...
			<view class="goods_reco" style="margin-top: 10px;display: flex">
				...
				<view class="goods_desc" style="padding-bottom: 8px;padding-top:8px;width: 80%">{{ goods.goodsDesc }}
				</view>
			</view>
			...
			loadGoodsInfo: function() {
				...
				this.$Request.getT('/pdd/details/' + this.itemid).then(res => {
					if (res.goodsDetailResponse) {
						...
						if (grade) {
							that.money = ((((item.minGroupPrice - item.couponDiscount) * 0.01) * (item.promotionRate * 0.001)) *
			...
			copyTklWenAns: function() {
				...
				uni.setClipboardData({
					data: this.goods.goodsName +
						...
						'元\n【券后价】' +
						((this.goods.this.goods.couponDiscount) * 0.01).toFixed(2) +
						'元\n【必买理由】\n' +
