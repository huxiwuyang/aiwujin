    ...
    <view style="text-align: left">
        <view v-for="(item, index) in list" :key="index" class="item">
            <view style="display: flex">
                ...
                <view>
					<view style="margin-bottom: 4px;color: #FF6DB2;"> <text style="color: #000000;margin-right: 2px;"></text>{{item[3]?'会员':'游客'}}</view>
                    <view style="margin-bottom: 4px;color: black"> 昵称 {{item[2]}}</view>
                    <view style="margin-bottom: 4px"> 时间 {{item[0]}}</view>
					<view style="margin-bottom: 4px;color: #FF6DB2;" v-if="item[3]"> <text style="color: #000000;margin-right: 2px;">会员编号</text> {{item[3]}}</view>
                </view>
            </view>
           <!-- <view style="width: 30%;text-align: center;">
                <view v-if="item[4]===0"
                      style="text-align: center;color: white;margin-bottom: 4px;font-weight: 500;background: #FF6DB2;padding: 4px 8px;border-radius: 12px;margin-right: 30px">
                    未结算
                </view>
