<template>
	<view class="index-content">
		...
		<view style="background: #FFFFFF;">
			<view class="navbar" style="position: fixed;z-index: 100;text-align: center;width: 100%;margin-top:70upx">
				<view v-for="(item, index) in tabList" :key="index" class="nav-item" :class="{current: fromTabIndex === index}"
				 @tap="fromTabSelect(index)">
					{{item.name}}
				</view>
			</view>
			<scroll-view scroll-x class="bg-white nav" scroll-with-animation style="position: fixed;z-index: 100;text-align: center;width: 100%;margin-top: 150upx;border-bottom: 2upx solid ghostwhite">
				<view class="cu-item" :class="index==TabCur?'text-green cur':''" v-for="(item,index) in tab" @tap="tabSelect"
				 :data-id="index">
					<text :style="index==TabCur?'background: #FC3F78;color: white;padding: 4upx 20upx 4upx 20upx;border-radius: 32upx;margin-right: -20upx':'background: #ffffff;color: #333333;margin-right: -10px'">
						{{item.name}}
					</text>
				</view>
			</scroll-view>
		...
		<view style="background: #FFFFFF;">
			...
			<scroll-view scroll-x class="bg-white nav" scroll-with-animation style="border-top: 1px solid #F8F8F8;position: fixed;z-index: 100;text-align: center;width: 100%;margin-top: 84upx">
				<view class="cu-item" :class="index==TabCur?'text-green cur':''" v-for="(item,index) in tab" @tap="tabSelect"
				 :data-id="index">
					<text :style="index==TabCur?'background: #FC3F78;color: white;padding: 4upx 20upx 4upx 20upx;border-radius: 32upx;margin-right: -10px':'background: #ffffff;color: #333333;margin-right: -10px'">
						{{item.name}}
					</text>
				</view>
			</scroll-view>
		...
		<view class="index-coupon has-bg-white has-pd-10 top_30">
			<view class="goods-list" v-if="couponlist.length > 0&&fromTabIndex==0" style="padding-top: 230upx">
				<orange-goods-card-home v-for="(item,index) in couponlist" :index="index%2" :tkmoney='item.tkmoney' :tkmoneys='item.tkmoneys'
				 :itemid="item.itemid" :logo="logo" :isEnable='isEnable' :is-invitation="isInvitation" :itempic="item.itempic+'_310x310.jpg'"
